...
def main(user, passwd, step):
    ...
    if step == '':
        print ("已设置为随机步数（20848-22326）")
        step = str(random.randint(20848,22326))
    login_token = 0
...
if __name__ ==  "__main__":
	...
	# 简单写写说明 赠与有缘人
# 只需要修改 用户名 和密码 为自己的账号信息
# 本py脚本会打印login_token 运行完后自行复制出来
# lxk大佬的docker版本  cd scritps && cp backUp/xmSports.js ./xx_xmSports.js
# 修改 xx_xmSports.js 的内容 
# 找到 下面这两行的位置  修改login_token 为刚才打印的login_token
## let login_token = '';
##//需要修改的运动步数波动范围，脚本默认修改步数范围为1w9到2w5

#以后就执行 xx_xmSports 就行了。
