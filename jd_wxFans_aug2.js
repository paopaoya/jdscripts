...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        if (cookie) {
            ...
            await getActinfo()
            if ($.taskList && !$.fd3) {
                if ($.energy < 100) await getask();await getActinfo(true)
                console.log("去抽福袋...")
...
async function getask() {
    let taskArr = ["task1Sign", "task2BrowGoods", "task3AddCart"]
    for (o = 0; o < taskArr.length; o++) {
        ...
        if (task.upLimit == task.finishedCount) {
            ...
            if (o == 2) {
                for (bgoods of task.taskGoodList.slice(0, 3)) {
                    await $.wait(1000);
