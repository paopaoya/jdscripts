 ...
 *
 * https://lzkjdz-isv.isvjcloud.com/wxFansInterActionActivity/activity/31073025b8a34de59d8d55faffdd44ab?activityId=31073025b8a34de59d8d55faffdd44ab
 *
 * cron: 随便吧~ 一天跑个两次
 */
...
let actid = "162c43699ba945e8adb83b2bd5fe0142";
let uuuid = "3307706";
...
function getActCk() {
    return new Promise(resolve => {
        $.get(taskUrl("/wxFansInterActionActivity/activity/" + uuuid, `activityId=${actid}&shareuserid4minipg=jd_4806fb66e0f3e&shopid=undefined`), (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
function getshopid() {
    let config = taskPostUrl("/customer/getSimpleActInfoVo", `activityId=${actid}`)
    return new Promise(resolve => {
        $.post(config, async (err, resp, data) => {
            try {
                if (err) {
...
function adlog() {
    let config = taskPostUrl("/common/accessLogWithAD", `venderId=${$.shopid}&code=69&pin=${encodeURIComponent($.pin)}&activityId=90121061401&pageUrl=https://lzkjdz-isv.isvjcloud.com/wxFansInterActionActivity/activity/${uuuid}?activityId=${actid}&shareuserid4minipg=jd_4806fb66e0f3e&shopid=undefined&subType=app&adSource=`)
    return new Promise(resolve => {
        $.post(config, async (err, resp, data) => {
            try {
                if (err) {
...
function getActinfo(end) {
    return new Promise(resolve => {
        let body = `activityId=${actid}&pin=${encodeURIComponent($.pin)}`
        $.post(taskPostUrl('/wxFansInterActionActivity/activityContent', body), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
                    ...
                    if (data.result) {
                        ...
                        if (actinfo) {
                            ...
                            $.fd3 = actorinfo.prizeThreeStatus
...
// 做任务
function dotask(id, body) {
    return new Promise(resolve => {
        $.post(taskPostUrl(`/wxFansInterActionActivity/${id}`, `activityId=${actid}&uuid=${$.uuid}${body ? body : ""}`), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
function taskUrl(url, body) {
    ...
    //  console.log(cookie)
    return {
        url: `https://lzkjdz-isv.isvjcloud.com${url}?${body}`,
        headers: {
            'Host': 'lzkjdz-isv.isvjcloud.com',
            'Accept': 'application/json',
            'X-Requested-With': 'XMLHttpRequest',
            'Referer': 'https://lzkjdz-isv.isvjcloud.com/wxFansInterActionActivity/activity/' + uuuid + '?activityId=' + actid + '&shareuserid4minipg=jd_4806fb66e0f3e&shopid=undefined',
...
function taskPostUrl(url, body) {
    return {
        url: `https://lzkjdz-isv.isvjcloud.com${url}`,
        body: body,
        headers: {
            'Host': 'lzkjdz-isv.isvjcloud.com',
            'Accept': 'application/json',
